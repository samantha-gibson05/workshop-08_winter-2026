...
# create clean data set using drought_exp
drought_exp_clean <- drought_exp |>
  # cleans column headers and makes format the same
  clean_names() |>
  # creates new column with species name
  ...
  # creates columns with water treatment name
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |>  
  # makes species name and water treatment a factor
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |>
  # selects only the follwoing columns
...
```{r}
drought_exp_clean |> slice_sample(n = 10)
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = 'Specific Leaf Area (cm^2^/g)',
      y = 'Total Mass (g)',
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = 'Water Treatment Type',
       y = 'Total Mass (g)',
       title = 'Well Watered Plants have Greater Mean Mass')
```
...
```{r}
drought_exp_clean_ordered <- drought_exp_clean |>
  group_by(species_name) |>
  mutate(mean_mass = mean(total_g, na.rm = TRUE)) |>
  ungroup() |>
  mutate(species_name = fct_reorder(species_name, mean_mass, .desc = TRUE))

ggplot(data = drought_exp_clean_ordered,
       aes(x = species_name, 
           y = total_g)) +
  geom_boxplot() +
  geom_jitter() +
  ...
  labs(x = 'Species Name',
       y = 'Total Mass (g)',
       title = 'Mean Total Plant Mass Differs Across Species')
```
